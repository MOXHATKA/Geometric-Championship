...
export default function flowSystem(scene: Phaser.Scene): void {
    ...
    movementQuery.forEach((id) => {
        ...
        const time = Sprite.collisionTime[id];
        Sprite.collisionTime[id] -= 1;
        if(time > 2) return;
        ...
        if (
            Target.sprite[id].x != 0 &&
            Target.sprite[id].x != 0 &&
            (Math.abs(vect.x) > 0.5 || Math.abs(vect.y) > 0.5)
        ) {
            ...
            // sprite.setPosition((sprite.x + vect.x * 12), (sprite.y + vect.y * 12));
        }
    });
}
